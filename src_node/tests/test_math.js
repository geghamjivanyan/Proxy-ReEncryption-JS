...
function test_scalar_add(){
    var sc_1 = proxy.Scalar.generate_random();
    var sc_2 = proxy.Scalar.generate_random();

    var sc_12 = sc_1.add(sc_2); // sc_1 + sc_2
    var sc_21 = sc_2.add(sc_1); // sc_2 + sc_1

    assert(!sc_12.eq(sc_21), "FAILED: Scalar Add"); // sc_12 == sc_21
}
...
function test_scalar_sub(){
    ...
    assert(sc_2.eq(sc_2_back), "FAILED: Scalar Sub 1");

    var sc_3 = proxy.Scalar.generate_random();
    ...
    assert(sc_1.eq(sc_1_back), "FAILED: Scalar Sub 2");
}
...
function test_scalar_mul(){
    ...
    assert(sc_12.eq(sc_21), "FAILED: Scalar Mul"); // sc_12 == sc_21
}

function test_group_element_add(){
    var ge_1 = proxy.GroupElement.generate_random();
    var ge_2 = proxy.GroupElement.generate_random();
    var ge_12 = ge_1.add(ge_2); // ge_1 + ge_2
    var ge_21 = ge_2.add(ge_1); // ge_2 + ge_1

    assert(ge_12.eq(ge_21), "FAILED: GroupElement Add"); // ge_12 == ge_21 
}
